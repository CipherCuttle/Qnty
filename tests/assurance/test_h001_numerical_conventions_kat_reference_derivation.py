...
SHA = "28551aa041aff2985e0023e61516b08f528d93cf72c23c8c3541793f6f61c691"
...
def _gamma(values, mean, j):
    ...
    total = 0.0
    for t in range(j, n):
        left_centered = values[t] - mean
        if not _isfinite(left_centered):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        right_centered = values[t - j] - mean
        if not _isfinite(right_centered):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        product = left_centered * right_centered
        if not _isfinite(product):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        total += product
        if not _isfinite(total):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    gamma = total / n
    if not _isfinite(gamma):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    return gamma
...
def _hac(values, lag=1):
    ...
    for j in range(1, lag + 1):
        ...
        if not _isfinite(weight):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        weighted_gamma = weight * gamma[j]
        if not _isfinite(weighted_gamma):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        doubled_term = 2.0 * weighted_gamma
        if not _isfinite(doubled_term):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        omega = omega + doubled_term
...
def _centered_num(xbar_star, xbar):
    if not _isfinite(xbar_star) or not _isfinite(xbar):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    centered_num = xbar_star - xbar
    if not _isfinite(centered_num):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    return 0.0 if centered_num == 0.0 else centered_num
...
def test_hac_non_finite_intermediate_fail_closed():
    ...
    with pytest.raises(ValueError, match="NON_FINITE_INTERMEDIATE"):
        _mean([1.7976931348623157e+308, 1.7976931348623157e+308])
...
def test_centering():
    ...
    assert _centered_num(_mean([x[p] for p in path]), _mean(x)) == -0.5
...
def test_studentization():
    ...
    centered_num = _centered_num(_mean(xs), _mean(x))
    assert _stud(centered_num, boot) == -1.2060453783110545
    assert _stud(centered_num, observed) == -2.0
...
def test_gamma_product_guard_precedes_accumulation():
    ...
    term_idx = next(i for i, s in enumerate(body) if _assign_target_name(s) == "product")
    guard_idx = term_idx + 1
    assert guard_idx < len(body), "no statement follows the per-term product assignment"
    assert _is_finiteness_guard_on(body[guard_idx], "product"), (
    ...
    assert _is_accumulation_of(body[acc_idx], "total", "product"), (
...
def test_named_intermediates_are_immediately_guarded():
    """AST proof for each governed scalar: bind, immediate exact guard, then use."""
    cases = ((_mean, ("total", "result")), (_gamma, ("left_centered", "right_centered", "product", "total", "gamma")), (_hac, ("ratio", "weight", "weighted_gamma", "doubled_term", "omega")), (_centered_num, ("centered_num",)))
    for function, names in cases:
        tree = ast.parse(inspect.getsource(function)).body[0]
        bodies = [tree.body]
        bodies.extend(node.body for node in ast.walk(tree) if isinstance(node, (ast.For, ast.While)))
        statements = [s for body in bodies for s in body]
        for name in names:
            index = next(i for i, statement in reversed(list(enumerate(statements))) if _assign_target_name(statement) == name or (isinstance(statement, ast.AugAssign) and isinstance(statement.target, ast.Name) and statement.target.id == name))
            assert _is_finiteness_guard_on(statements[index + 1], name), (function.__name__, name)
    ...
    try:
        if gap == "HAC_AUTOCOVARIANCE_AND_STANDARD_ERROR_CONVENTION":
            ...
            if "centered_numerator" in expected:
                _check("centered_numerator", _centered_num(xbar_star, xbar))
...
def _dispatch_e2e(fid, inp, params, expected, _check, _check_raw):
    ...
    for path in paths:
        ...
        for name in names:
            ...
            tstars.append(_stud(_centered_num(mean_s, mean), se_s))
...
# --- Phase B repair: ordering / vacuous-recording structural guards --------
#
# PHASE_B_MUTANT_39 proved that marking a fixture into _EXERCISED before its
# dispatch call completes is not observable by outcome alone (dispatch either
# still fails on its own, or still succeeds, independent of marker order).
# PHASE_B_MUTANT_40 proved that stripping the value-comparison Assert out of
# _check/_check_raw while still calling asserted.add(key) is likewise
# unobservable when every currently-registered fixture value already happens
# to be correct. Both are genuine coverage gaps in pure black-box execution;
# these two structural (AST-order) tests close them the same way the
# existing product-guard-ordering tests close the analogous arithmetic gap.


def test_dispatch_precedes_exercised_marking():
    source = inspect.getsource(test_fixture_dispatch_execution_coverage)
    body = ast.parse(source).body[0].body
    dispatch_idx = next(
        i for i, s in enumerate(body)
        if isinstance(s, ast.Expr) and isinstance(s.value, ast.Call)
        and isinstance(s.value.func, ast.Name) and s.value.func.id == "_dispatch_fixture"
    )
    mark_idx = next(
        i for i, s in enumerate(body)
        if isinstance(s, ast.Expr) and isinstance(s.value, ast.Call)
        and isinstance(s.value.func, ast.Attribute) and s.value.func.attr == "add"
    )
    assert dispatch_idx < mark_idx, "a fixture must be dispatched before it is marked _EXERCISED"


def test_check_helpers_compare_before_recording():
    source = inspect.getsource(_dispatch_fixture)
    tree = ast.parse(source).body[0]
    helpers = [n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name in ("_check", "_check_raw")]
    assert {h.name for h in helpers} == {"_check", "_check_raw"}
    for helper in helpers:
        asserts_before_add = 0
        add_seen = False
        for stmt in helper.body:
            ...
            if isinstance(stmt, ast.Assert):
                asserts_before_add += 1
        assert add_seen, (helper.name, "no asserted.add(key) call found")
        assert asserts_before_add >= 2, (
            helper.name, "expected >=2 asserts (field-membership + value-comparison) before recording"
        )
