...
def covariance(a, j):
    ...
    while t < n:
        left_centered = a[t] - m
        if not _finite(left_centered):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        right_centered = a[t - j] - m
        if not _finite(right_centered):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
...
def long_run_variance(a, lag):
    ...
    for j in range(1, lag + 1):
        ...
        if not _finite(weight):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        weighted = weight * gammas[j]
        if not _finite(weighted):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
        term = 2.0 * weighted
        if not _finite(term):
            raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
...
def centered_numerator(xbar_star, xbar):
    if not _finite(xbar_star) or not _finite(xbar):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    centered = xbar_star - xbar
    if not _finite(centered):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE")
    return 0.0 if centered == 0.0 else centered
...
def test_centering():
    ...
    assert (mean(a), mean(resampled), centered_numerator(mean(resampled), mean(a))) == (2.0, 1.5, -0.5)
...
def test_studentization():
    ...
    assert o == 0.414578098794425 and ratio(centered_numerator(mean(b), mean(a)), o) == -1.2060453783110545
...
def test_covariance_product_guard_precedes_accumulation():
    ...
    bind_idx = next(i for i, (kind, _) in enumerate(tags) if kind == "bind:product")
    guard_idx = bind_idx + 1
    assert guard_idx < len(tags), "no statement follows the per-term product binding"
    assert tags[guard_idx][0] == "guard:product", (
    ...
    assert tags[accum_idx][0].startswith("accum_candidate:total:product"), (
...
def test_each_named_intermediate_has_an_adjacent_raising_guard():
    """Independent AST classification over each governed helper."""
    cases = ((mean, ("total", "result")), (covariance, ("left_centered", "right_centered", "product", "total", "gamma")), (long_run_variance, ("fraction", "weight", "weighted", "term", "omega")), (centered_numerator, ("centered",)))
    for function, names in cases:
        node = ast.parse(inspect.getsource(function)).body[0]
        blocks = [node.body] + [child.body for child in ast.walk(node) if isinstance(child, (ast.For, ast.While))]
        tags = [tag for block in blocks for tag in _classify_body_statements(block)]
        for name in names:
            index = next(i for i, tag in reversed(list(enumerate(tags))) if tag[0] == f"bind:{name}" or tag[0].startswith(f"accum_candidate:{name}:"))
            assert tags[index + 1][0] == f"guard:{name}", (function.__name__, name, tags[index:index + 2])
...
def _run_centering_gap(fid, inp, params, expected, error_category, _bind, _bind_raw):
    ...
    if "centered_numerator" in expected:
        _bind("centered_numerator", centered_numerator(m_star, m))
...
def _run_stud_gap(fid, inp, params, expected, error_category, _bind, _bind_raw):
    ...
    if "resample_path" in inp and "x" in inp:
        ...
        numerator = centered_numerator(m_star, m)
...
#
# Independent-derivation counterpart of the reference file's Phase B repair:
# closes the same two black-box-unobservable gaps (marking a fixture into
# _LEDGER before its dispatch completes; recording a field as asserted in
# _bind/_bind_raw without actually comparing it) via AST-order structural
# checks, using this file's own dispatch-table-based dispatcher.


def test_dispatch_precedes_ledger_marking():
    ...
    dispatch_idx = next(
        ...
        and isinstance(s.value.func, ast.Name) and s.value.func.id == "_dispatch"
    )
    mark_idx = next(
        i for i, s in enumerate(body)
        if isinstance(s, ast.Expr) and isinstance(s.value, ast.Call)
    ...
    assert dispatch_idx < mark_idx, "a fixture must be dispatched before it is marked in _LEDGER"


def test_bind_helpers_compare_before_recording():
    source = inspect.getsource(_dispatch)
    tree = ast.parse(source).body[0]
    helpers = [n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name in ("_bind", "_bind_raw")]
    assert {h.name for h in helpers} == {"_bind", "_bind_raw"}
