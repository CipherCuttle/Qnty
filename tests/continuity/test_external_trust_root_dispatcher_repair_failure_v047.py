...
from quantbot.continuity import external_trust_root_dispatcher_repair_failure_v047 as v047
...
def _write(root, receipt):
    path = root / v047.HANDOFF_RELPATH
...
def _receipt(root):
    return json.loads((root / v047.HANDOFF_RELPATH).read_bytes())
...
def test_v047_baseline_verifies(tmp_path):
    context.load_and_verify_continuity_state(_tree(tmp_path))


@pytest.mark.parametrize(("path", "value"), [
    ("v046_failed_review_binding.commit", "0" * 40),
    ("v046_failed_review_binding.tree", "0" * 40),
    ("v046_failed_review_binding.review_outcome", "PASS"),
    ("v046_failed_review_binding.finding.id", "H000"),
    ("v046_failed_review_binding.finding.hostile_result.authoritative", False),
    ("v046_failed_review_binding.released", True),
...
def test_v047_rejects_finding_or_authority_drift(tmp_path, path, value):
    root = _tree(tmp_path)
