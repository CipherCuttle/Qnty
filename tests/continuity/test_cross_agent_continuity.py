...
def test_calibration_governance_production_state_renders_authorized_but_unfrozen():
    state = load_and_verify_continuity_state(ROOT)
    if state["active_task"]["phase"] in (context._H001_CALIBRATION_CANDIDATE_PHASE, context._H001_CALIBRATION_REREVIEW_PHASE, context._H001_CALIBRATION_EFFECTIVE_PHASE, context._H001_CALIBRATION_EXECUTION_GOVERNANCE_PHASE, context._H001_CALIBRATION_IMPLEMENTATION_BLOCKED_PHASE):
...
def test_h001_calibration_execution_governance_production_state_is_implementation_only():
    state = load_and_verify_continuity_state(ROOT)
    if state["active_task"]["phase"] == context._H001_CALIBRATION_IMPLEMENTATION_BLOCKED_PHASE:
        pytest.skip("production tree has recorded the engine implementation block")
...
def test_h001_calibration_implementation_blocked_production_state_is_exact():
    ...
    assert state["handoff_receipt"]["receipt_index"] == 24
    assert state["handoff_receipt"]["numerical_convention_gap_inventory"] == context._H001_CALIBRATION_IMPLEMENTATION_BLOCKED_GAPS
    packet = render_context_packet(state)
    for marker in (
...
@pytest.mark.parametrize("mutate", [
    ...
    lambda r: r["changed_file_scope"].pop(),
    lambda r: r["changed_file_scope"].reverse(),
    lambda r: r["numerical_convention_gap_inventory"].pop(),
    lambda r: r["numerical_convention_gap_inventory"].append("EXTRA_GAP"),
    lambda r: r["numerical_convention_gap_inventory"].reverse(),
    lambda r: r["decisions"].append("HAC_AUTOCOVARIANCE_FORMULA=SELECTED"),
    lambda r: r["decisions"].append("BOOTSTRAP_CENTERING_FORMULA=SELECTED"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_IMPLEMENTATION=NOT_IMPLEMENTED"), "H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_IMPLEMENTATION=PARTIALLY_IMPLEMENTED"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_IMPLEMENTATION=NOT_IMPLEMENTED"), "H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_IMPLEMENTATION=IMPLEMENTED"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_ENGINE_IMPLEMENTATION_COMMIT=NONE"), "H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_ENGINE_IMPLEMENTATION_COMMIT=abc"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_ENGINE_IMPLEMENTATION_PR=NONE"), "H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION_ENGINE_IMPLEMENTATION_PR=123"),
    lambda r: r["next_actions"].__setitem__(0, "IMPLEMENT_H001_SYNTHETIC_NULL_CALIBRATION_NUMERICAL_CONVENTIONS_AMENDMENT_CANDIDATE_FOR_INDEPENDENT_REVIEW"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION=NOT_AUTHORIZED"), "H001_SYNTHETIC_NULL_CALIBRATION_EXECUTION=AUTHORIZED"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_SYNTHETIC_NULL_CALIBRATION_RESULTS=NONE"), "H001_SYNTHETIC_NULL_CALIBRATION_RESULTS=AVAILABLE"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_EXECUTION=0/0"), "H001_EXECUTION=1/1"),
    lambda r: r["decisions"].__setitem__(r["decisions"].index("H001_REAL_DATA_ACCESS=FORBIDDEN"), "H001_REAL_DATA_ACCESS=AUTHORIZED"),
    ...
    lambda r: r["safety_state"].update(paper_trade_authorized=True),
    lambda r: r["safety_state"].update(live_integration_authorized=True),
    lambda r: r["blockers"].remove("H001 synthetic calibration engine implementation is blocked by incomplete result-determinative numerical conventions"),
    lambda r: r["blockers"].remove("H001 synthetic calibration execution remains unauthorized"),
    lambda r: r["blockers"].append("H001 calibration specification remains unfrozen"),
])
def test_h001_calibration_implementation_blocked_mutations_fail_closed(tmp_path, mutate):
    with pytest.raises(ValueError):
        load_and_verify_continuity_state(_calibration_implementation_blocked_mutated_tree(tmp_path, mutate_receipt=mutate))


def test_h001_calibration_implementation_blocked_active_pointer_and_no_git_fail_closed(tmp_path):
    root = _calibration_implementation_blocked_mutated_tree(
        tmp_path, mutate_active=lambda a: a.update(handoff_receipt_sha256="0" * 64),
    )
    with pytest.raises(ValueError):
        load_and_verify_continuity_state(root)
