...
H001_NC_FIXTURE_IDS = {
    ...
    "KAT-HAC-ORDER-001", "KAT-HAC-DIVISION-001",
    "KAT-HAC-NONFINITE-INTERMEDIATE-MEAN-001", "KAT-HAC-NONFINITE-INTERMEDIATE-GAMMA-001",
    "KAT-CENTER-001",
    "KAT-STUD-VALID-001", "KAT-STUD-ZEROSE-ZEROMEAN-001", "KAT-STUD-ZEROSE-NONZERO-001", "KAT-STUD-OBS-VS-BOOT-001",
    "KAT-STUD-NONFINITE-INTERMEDIATE-001", "KAT-STUD-NEGATIVE-SE-001", "KAT-STUD-SIGNED-ZERO-001",
...
def _studentize_t_statistic(numerator, se):
    """Studentize a t-statistic with explicit non-finite and sign guards.

    This is a contract-level helper only; it is NOT the governed KAT
    derivation and its tests are not sufficient evidence on their own that
    the two governed KAT files (test_h001_numerical_conventions_kat_*) apply
    the same fail-closed behavior -- each of those files independently
    implements and executes its own studentization function.

    Required behavior per the numerical conventions contract:
    - non-finite numerator -> ValueError (H001_NC_NON_FINITE_INTERMEDIATE)
    - non-finite SE -> ValueError (H001_NC_NON_FINITE_INTERMEDIATE)
    - SE < 0 -> ValueError (H001_NC_NEGATIVE_STANDARD_ERROR), a category
      distinct from and checked before the zero-SE category
    - SE == 0 and numerator == 0 -> the literal +0.0
    - SE == 0 and numerator != 0 -> ValueError (H001_NC_ZERO_STANDARD_ERROR)
    - SE > 0 -> numerator / se, canonicalized to +0.0 if the quotient is zero,
      and rejected as H001_NC_NON_FINITE_INTERMEDIATE if the quotient overflows
    ...
    if not (numerator <= 1e308 and numerator >= -1e308 and numerator == numerator):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE: non-finite numerator")
    if not (se <= 1e308 and se >= -1e308 and se == se):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE: non-finite standard error")
    if se < 0:
        raise ValueError("H001_NC_NEGATIVE_STANDARD_ERROR: negative standard error")
    if se == 0.0:
        if numerator == 0.0:
            return 0.0
        raise ValueError("H001_NC_ZERO_STANDARD_ERROR: zero standard error with non-zero numerator")
    result = numerator / se
    if not (result <= 1e308 and result >= -1e308 and result == result):
        raise ValueError("H001_NC_NON_FINITE_INTERMEDIATE: non-finite quotient")
