...
def _latest_heartbeat_ts(hb_path: Path) -> tuple[str, str | None]:
    ...
    status:
      ...
      - "ok"        : file present and parsed; ts is the newest bar_processed_at (may be None
                      if no record carried one — caller treats that as malformed/fail-closed).
      - "malformed" : file present but a line is not valid JSON, or it cannot be read.
...
def check_freshness(
    ...
    Every consumed row (>= forward_start_ts, or all rows if None) is fully validated — each
    must carry the full observer contract, a list `active_symbols`, and a parseable on-grid,
    non-future timestamp. Any violation aborts; stale/malformed/missing/future output is
    ...
    STALE_OBSERVATION, MALFORMED_HEARTBEAT, STALE_HEARTBEAT.
    """
    ...
    # Validate EVERY consumed row in full — not just the final row. A malformed/off-grid
    # row anywhere in the consumed set must abort (Blocker 3).
    seen_ts: set[str] = set()
    consumed_count = 0
    ...
    for i, row in enumerate(per_bar):
        ...
        if not isinstance(active, list):
            return FreshnessResult(
                False,
                "MALFORMED_OBSERVATION_LOG",
                f"per_bar_obs[{i}] active_symbols must be a list (got {type(active).__name__})",
            )

        ts = row.get("timestamp")
        # Only rows in the consumed forward set need to be on-grid / non-future; pre-forward
        # rows are not processed. When forward_start_ts is None, every row is consumed.
        if forward_start_ts is not None and isinstance(ts, str) and ts < forward_start_ts:
            continue
        consumed_count += 1
        ...
        if ts in seen_ts:
            return FreshnessResult(
                ...
                f"per_bar_obs has a duplicate consumed timestamp {ts!r}; refusing to process "
    ...
    if consumed_count == 0:
        # Nothing at/after forward_start_ts yet — not stale, just no forward bars to process.
        latest_ts = per_bar[-1].get("timestamp")
        return _ok(latest_ts if isinstance(latest_ts, str) else "", None)

    latest_ts = latest_consumed_ts
