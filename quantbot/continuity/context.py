...
# BEGIN H001 RNG RUNTIME ACTIVATION V029 APPEND
# This extension is intentionally appended so v028 reconstruction can truncate
# at this marker and recover byte-identical historical sources without Git.
from quantbot.continuity import h001_rng_runtime_activation_v029 as _v029

_validate_receipt_v028 = _validate_receipt


def _validate_v029_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "rng_runtime_candidate_resolved_inventory", "candidate_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 29:
        _fail("H001 RNG-runtime activation receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]):
        _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v029.PHASE:
        return _validate_receipt_v028(parsed, active, root)
    _validate_v029_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]:
        _fail("handoff_receipt identity does not match active_task")
    _v029.validate(parsed, root)
    _cross_check_artifact_records(parsed, root)
    _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"])
    return parsed


_render_context_packet_v028 = render_context_packet


def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v028(state)
    if state["active_task"]["phase"] != _v029.PHASE:
        return packet
    lines = packet.splitlines()
    insert_at = next((index for index, line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines))
    lines[insert_at:insert_at] = _v029.DECISIONS
    return "\n".join(lines)


# END H001 RNG RUNTIME ACTIVATION V029 APPEND


# BEGIN H001 RNG RUNTIME ACTIVATION V029 PREDECESSOR APPEND
# v028 has the same append-only receipt shape as v027; recognize it while
# reconstructing the immutable predecessor chain without consulting Git.
_validate_predecessor_chain_v028 = _validate_predecessor_chain
...
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    ...
    while True:
        ...
        if hashlib.sha256(target.read_bytes()).hexdigest() != _require_sha256(predecessor["sha256"], "predecessor sha256"):
            _fail(f"predecessor receipt {path!r} bytes do not match the recorded sha256")
        previous = _load_canonical_document(target.read_bytes(), "predecessor receipt")
        if previous.get("receipt_index") == 28:
            _validate_v028_receipt_body(previous)
            previous_index = 28
        elif previous.get("receipt_index") == 27:
            _validate_v027_receipt_body(previous, root)
            previous_index = 27
        else:
            previous_index = _validate_receipt_body(previous, "predecessor receipt", root, verify_evidence_files=False)
...
# END H001 RNG RUNTIME ACTIVATION V029 PREDECESSOR APPEND


# BEGIN H001 NUMERICAL CONVENTIONS CANDIDATE V030 APPEND
# Appended so v029 reconstruction can truncate at this marker and recover
# byte-identical historical sources without Git.
from quantbot.continuity import h001_numerical_conventions_candidate_v030 as _v030

_validate_receipt_v029 = _validate_receipt


def _validate_v030_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "candidate_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 30:
        _fail("H001 numerical-conventions candidate receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]):
        _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1)
    ...
    if type(parsed["next_actions"]) is not list or len(parsed["next_actions"]) != 1:
        _fail("handoff_receipt next_actions must contain exactly one action")
    if parsed["rng_runtime_candidate_resolved_inventory"] != _v030.previous.DOMAINS:
        _fail("H001 numerical-conventions candidate inherited RNG resolved-domain inventory drifted")


def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v030.PHASE:
        return _validate_receipt_v029(parsed, active, root)
    _validate_v030_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]:
        _fail("handoff_receipt identity does not match active_task")
    _v030.validate(parsed, root)
    _cross_check_artifact_records(parsed, root)
    _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"])
    return parsed


_render_context_packet_v029 = render_context_packet


def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v029(state)
    if state["active_task"]["phase"] != _v030.PHASE:
        return packet
    lines = packet.splitlines()
    insert_at = next((index for index, line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines))
    lines[insert_at:insert_at] = _v030.DECISIONS
    return "\n".join(lines)


# END H001 NUMERICAL CONVENTIONS CANDIDATE V030 APPEND


# BEGIN H001 NUMERICAL CONVENTIONS CANDIDATE V030 PREDECESSOR APPEND
# v029 has an append-only receipt shape with extra keys; recognize it while
# reconstructing the immutable predecessor chain without consulting Git.
_validate_predecessor_chain_v029 = _validate_predecessor_chain
...
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    ...
    while True:
        ...
        if hashlib.sha256(target.read_bytes()).hexdigest() != _require_sha256(predecessor["sha256"], "predecessor sha256"):
            _fail(f"predecessor receipt {path!r} bytes do not match the recorded sha256")
        previous = _load_canonical_document(target.read_bytes(), "predecessor receipt")
        if previous.get("receipt_index") == 29:
            _validate_v029_receipt_body(previous)
            previous_index = 29
        elif previous.get("receipt_index") == 28:
            _validate_v028_receipt_body(previous)
            previous_index = 28
        elif previous.get("receipt_index") == 27:
            _validate_v027_receipt_body(previous, root)
...
# END H001 NUMERICAL CONVENTIONS CANDIDATE V030 PREDECESSOR APPEND


# BEGIN H001 NUMERICAL CONVENTIONS REVIEW COMPLETION V031 APPEND
# This extension is intentionally appended so v030 reconstruction can truncate
# at this marker and recover byte-identical historical sources without Git.
from quantbot.continuity import h001_numerical_conventions_review_completion_v031 as _v031

_validate_receipt_v030 = _validate_receipt


def _validate_v031_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "candidate_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 31:
        _fail("H001 numerical-conventions review-completion receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]):
        _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v031.PHASE:
        return _validate_receipt_v030(parsed, active, root)
    _validate_v031_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]:
        _fail("handoff_receipt identity does not match active_task")
    _v031.validate(parsed, root)
    _cross_check_artifact_records(parsed, root)
    _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"])
    return parsed


_render_context_packet_v030 = render_context_packet


def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v030(state)
    if state["active_task"]["phase"] != _v031.PHASE:
        return packet
    lines = packet.splitlines()
    insert_at = next((index for index, line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines))
    lines[insert_at:insert_at] = _v031.DECISIONS
    return "\n".join(lines)


# END H001 NUMERICAL CONVENTIONS REVIEW COMPLETION V031 APPEND


# BEGIN H001 NUMERICAL CONVENTIONS REVIEW COMPLETION V031 PREDECESSOR APPEND
# v030 has an append-only receipt shape with extra keys; recognize it while
# reconstructing the immutable predecessor chain without consulting Git.
_validate_predecessor_chain_v030 = _validate_predecessor_chain
...
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    ...
    while True:
        ...
        if hashlib.sha256(target.read_bytes()).hexdigest() != _require_sha256(predecessor["sha256"], "predecessor sha256"):
            _fail(f"predecessor receipt {path!r} bytes do not match the recorded sha256")
        previous = _load_canonical_document(target.read_bytes(), "predecessor receipt")
        if previous.get("receipt_index") == 30:
            _validate_v030_receipt_body(previous)
            previous_index = 30
        elif previous.get("receipt_index") == 29:
            _validate_v029_receipt_body(previous)
            previous_index = 29
        elif previous.get("receipt_index") == 28:
            _validate_v028_receipt_body(previous)
...
# END H001 NUMERICAL CONVENTIONS REVIEW COMPLETION V031 PREDECESSOR APPEND

# BEGIN H001 NUMERICAL CONVENTIONS ACTIVATION V032 APPEND
# This extension is intentionally appended so v031 reconstruction can truncate
# at this marker and recover byte-identical historical sources without Git.
from quantbot.continuity import h001_numerical_conventions_activation_v032 as _v032

_validate_receipt_v031 = _validate_receipt


def _validate_v032_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "candidate_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 32:
        _fail("H001 numerical-conventions activation receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]):
        _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v032.PHASE:
        return _validate_receipt_v031(parsed, active, root)
    _validate_v032_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]:
        _fail("handoff_receipt identity does not match active_task")
    _v032.validate(parsed, root)
    _cross_check_artifact_records(parsed, root)
    _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"])
    return parsed


_render_context_packet_v031 = render_context_packet


def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v031(state)
    if state["active_task"]["phase"] != _v032.PHASE:
        return packet
    lines = packet.splitlines()
    insert_at = next((index for index, line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines))
    lines[insert_at:insert_at] = _v032.DECISIONS
    return "\n".join(lines)


# END H001 NUMERICAL CONVENTIONS ACTIVATION V032 APPEND


# BEGIN H001 NUMERICAL CONVENTIONS ACTIVATION V032 PREDECESSOR APPEND
# v031 has an append-only receipt shape with extra keys; recognize it while
# reconstructing the immutable predecessor chain without consulting Git.
_validate_predecessor_chain_v031 = _validate_predecessor_chain
...
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    ...
    while True:
        ...
        if hashlib.sha256(target.read_bytes()).hexdigest() != _require_sha256(predecessor["sha256"], "predecessor sha256"):
            _fail(f"predecessor receipt {path!r} bytes do not match the recorded sha256")
        previous = _load_canonical_document(target.read_bytes(), "predecessor receipt")
        if previous.get("receipt_index") == 31:
            _validate_v031_receipt_body(previous)
            previous_index = 31
        elif previous.get("receipt_index") == 30:
            _validate_v030_receipt_body(previous)
            previous_index = 30
        elif previous.get("receipt_index") == 29:
            _validate_v029_receipt_body(previous)
...
# END H001 NUMERICAL CONVENTIONS ACTIVATION V032 PREDECESSOR APPEND


# BEGIN H001 SYNTHETIC NULL CALIBRATION EXECUTION ENGINE IMPLEMENTATION V033 APPEND
# This extension is intentionally appended so v032 reconstruction can truncate
# at this marker and recover byte-identical historical sources without Git.
from quantbot.continuity import h001_synthetic_null_calibration_execution_engine_implementation_v033 as _v033

_validate_receipt_v032 = _validate_receipt


def _validate_v033_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "engine_implementation_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 33:
        _fail("H001 engine-implementation receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]):
        _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v033.PHASE:
        return _validate_receipt_v032(parsed, active, root)
    _validate_v033_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]:
        _fail("handoff_receipt identity does not match active_task")
    _v033.validate(parsed, root)
    _cross_check_artifact_records(parsed, root)
    _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"])
    return parsed


_render_context_packet_v032 = render_context_packet


def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v032(state)
    if state["active_task"]["phase"] != _v033.PHASE:
        return packet
    lines = packet.splitlines()
    insert_at = next((index for index, line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines))
    lines[insert_at:insert_at] = _v033.DECISIONS
    return "\n".join(lines)


# END H001 SYNTHETIC NULL CALIBRATION EXECUTION ENGINE IMPLEMENTATION V033 APPEND


# BEGIN H001 SYNTHETIC NULL CALIBRATION EXECUTION ENGINE IMPLEMENTATION V033 PREDECESSOR APPEND
# v032 has an append-only receipt shape with extra keys; recognize it while
# reconstructing the immutable predecessor chain without consulting Git.
_validate_predecessor_chain_v032 = _validate_predecessor_chain
...
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    ...
    while True:
        ...
        if hashlib.sha256(target.read_bytes()).hexdigest() != _require_sha256(predecessor["sha256"], "predecessor sha256"):
            _fail(f"predecessor receipt {path!r} bytes do not match the recorded sha256")
        previous = _load_canonical_document(target.read_bytes(), "predecessor receipt")
        if previous.get("receipt_index") == 32:
            _validate_v032_receipt_body(previous)
            previous_index = 32
        elif previous.get("receipt_index") == 31:
            _validate_v031_receipt_body(previous)
            previous_index = 31
        elif previous.get("receipt_index") == 30:
            _validate_v030_receipt_body(previous)
...
# END H001 SYNTHETIC NULL CALIBRATION EXECUTION ENGINE IMPLEMENTATION V033 PREDECESSOR APPEND

# BEGIN H001 SYNTHETIC NULL CALIBRATION ENGINE REVIEW COMPLETION V034 APPEND
from quantbot.continuity import h001_synthetic_null_calibration_execution_engine_implementation_review_completion_v034 as _v034

_validate_receipt_v033 = _validate_receipt

def _validate_v034_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "engine_implementation_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 34:
        _fail("H001 engine review-completion receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v034.PHASE: return _validate_receipt_v033(parsed, active, root)
    _validate_v034_receipt_body(parsed)
    ...
    _v034.validate(parsed, root); _cross_check_artifact_records(parsed, root); _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"]); return parsed

_render_context_packet_v033 = render_context_packet
def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v033(state)
    if state["active_task"]["phase"] != _v034.PHASE: return packet
    lines = packet.splitlines(); insert_at = next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines)); lines[insert_at:insert_at] = _v034.DECISIONS; return "\n".join(lines)

_validate_predecessor_chain_v033 = _validate_predecessor_chain
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    if parsed.get("receipt_index") != 34:
        return _validate_predecessor_chain_v033(parsed, root, receipt_relpath)
    predecessor = parsed["predecessor"]
    path = predecessor["path"]
    target = root / path
    if path != _v034.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != predecessor["sha256"]:
        _fail("H001 engine review-completion predecessor chain is wrong")
...
_validate_receipt_v034 = _validate_receipt

def _validate_v035_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "engine_implementation_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 35:
        _fail("H001 remaining-execution-pipeline scope-governance receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]): _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1); _validate_required_artifacts(parsed["required_artifacts"])
    for field in ("changed_file_scope", "blockers", "verified_commands", "prohibited_actions"): _require_str_list(parsed[field], f"handoff_receipt {field}", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v035.PHASE: return _validate_receipt_v034(parsed, active, root)
    _validate_v035_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]: _fail("handoff_receipt identity does not match active_task")
    _v035.validate(parsed, root); _cross_check_artifact_records(parsed, root); _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"]); return parsed

_render_context_packet_v034 = render_context_packet
def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v034(state)
    if state["active_task"]["phase"] != _v035.PHASE: return packet
    lines = packet.splitlines(); insert_at = next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines)); lines[insert_at:insert_at] = _v035.DECISIONS; return "\n".join(lines)

_validate_predecessor_chain_v034 = _validate_predecessor_chain
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    if parsed.get("receipt_index") != 35:
        return _validate_predecessor_chain_v034(parsed, root, receipt_relpath)
    predecessor = parsed["predecessor"]
    path = predecessor["path"]
    target = root / path
    if path != _v035.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != predecessor["sha256"]:
        _fail("H001 remaining-execution-pipeline scope-governance predecessor chain is wrong")
    _validate_predecessor_chain_v034(_load_canonical_document(target.read_bytes(), "predecessor receipt"), root, path)

# END H001 REMAINING EXECUTION PIPELINE SCOPE GOVERNANCE V035 APPEND

# BEGIN H001 PER-RUN COORDINATE/SEED ORCHESTRATION SELECTION V036 APPEND
from quantbot.continuity import h001_per_run_coordinate_and_seed_orchestration_selection_v036 as _v036

_validate_receipt_v035 = _validate_receipt

def _validate_v036_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "engine_implementation_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 36:
        _fail("H001 per-run coordinate/seed selection receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]): _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1); _validate_required_artifacts(parsed["required_artifacts"])
    for field in ("changed_file_scope", "blockers", "verified_commands", "prohibited_actions"): _require_str_list(parsed[field], f"handoff_receipt {field}", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v036.PHASE: return _validate_receipt_v035(parsed, active, root)
    _validate_v036_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]: _fail("handoff_receipt identity does not match active_task")
    _v036.validate(parsed, root); _cross_check_artifact_records(parsed, root); _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"]); return parsed

_render_context_packet_v035 = render_context_packet
def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v035(state)
    if state["active_task"]["phase"] != _v036.PHASE: return packet
    lines = packet.splitlines(); insert_at = next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines)); lines[insert_at:insert_at] = _v036.DECISIONS; return "\n".join(lines)

_validate_predecessor_chain_v035 = _validate_predecessor_chain
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    if parsed.get("receipt_index") != 36:
        return _validate_predecessor_chain_v035(parsed, root, receipt_relpath)
    predecessor = parsed["predecessor"]; path = predecessor["path"]; target = root / path
    if path != _v036.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != predecessor["sha256"]:
        _fail("H001 per-run coordinate/seed selection predecessor chain is wrong")
    _validate_predecessor_chain_v035(_load_canonical_document(target.read_bytes(), "predecessor receipt"), root, path)

# END H001 PER-RUN COORDINATE/SEED ORCHESTRATION SELECTION V036 APPEND

# BEGIN H001 PER-RUN COORDINATE/SEED ORCHESTRATION GOVERNANCE V037 APPEND
from quantbot.continuity import h001_per_run_coordinate_and_seed_orchestration_governance_v037 as _v037

_validate_receipt_v036 = _validate_receipt

def _validate_v037_receipt_body(parsed: dict) -> None:
    keys = set(_RECEIPT_KEYS) | {"phase", "current_transition_files", "numerical_convention_gap_inventory", "numerical_conventions_selected_convention_inventory", "rng_runtime_candidate_resolved_inventory", "engine_implementation_binding"}
    _require_exact_keys(parsed, keys, "handoff_receipt")
    if parsed["schema_version"] != "0.1.0" or parsed["receipt_kind"] != "qnty_cross_agent_handoff_receipt" or parsed["receipt_index"] != 37:
        _fail("H001 per-run coordinate/seed governance receipt structure is wrong")
    _require_str(parsed["source_branch"], "handoff_receipt source_branch")
    if type(parsed["source_head_commit"]) is not str or not _COMMIT_RE.fullmatch(parsed["source_head_commit"]): _fail("handoff_receipt source_head_commit must be a lowercase 40-hex commit")
    _require_str_list(parsed["decisions"], "handoff_receipt decisions", minimum=1); _validate_required_artifacts(parsed["required_artifacts"])
    for field in ("changed_file_scope", "blockers", "verified_commands", "prohibited_actions"): _require_str_list(parsed[field], f"handoff_receipt {field}", minimum=1)
...
def _validate_receipt(parsed: dict, active: dict, root: Path) -> dict:
    if active["phase"] != _v037.PHASE: return _validate_receipt_v036(parsed, active, root)
    _validate_v037_receipt_body(parsed)
    if parsed["task_id"] != active["task_id"] or parsed["protocol_id"] != active["protocol_id"]: _fail("handoff_receipt identity does not match active_task")
    _v037.validate(parsed, root); _cross_check_artifact_records(parsed, root); _validate_predecessor_chain(parsed, root, active["handoff_receipt_path"]); return parsed

_render_context_packet_v036 = render_context_packet
def render_context_packet(state: dict) -> str:
    packet = _render_context_packet_v036(state)
    if state["active_task"]["phase"] != _v037.PHASE: return packet
    lines = packet.splitlines(); insert_at = next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")), len(lines)); lines[insert_at:insert_at] = _v037.DECISIONS; return "\n".join(lines)

_validate_predecessor_chain_v036 = _validate_predecessor_chain
def _validate_predecessor_chain(parsed: dict, root: Path, receipt_relpath: str) -> None:
    if parsed.get("receipt_index") != 37:
        return _validate_predecessor_chain_v036(parsed, root, receipt_relpath)
    predecessor = parsed["predecessor"]; path = predecessor["path"]; target = root / path
    if path != _v037.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest() != predecessor["sha256"]:
        _fail("H001 per-run coordinate/seed governance predecessor chain is wrong")
...
# END H001 SYNTHETIC NULL CALIBRATION ENGINE REVIEW COMPLETION V034 APPEND
# BEGIN EXTERNAL TRUST-ROOT DISPATCHER REPAIR CANDIDATE V046 APPEND
from quantbot.continuity import external_trust_root_dispatcher_repair_candidate_v046 as _v046
_validate_receipt_v041 = _validate_receipt
def _validate_v046_receipt_body(parsed):
    keys=set(_RECEIPT_KEYS)|{"phase","current_transition_files","numerical_convention_gap_inventory","numerical_conventions_selected_convention_inventory","rng_runtime_candidate_resolved_inventory","engine_implementation_binding","per_run_coordinate_and_seed_orchestration_binding","v039_checkout_path_repair","v040_review_binding","v045_failed_review_binding","construction_lineage","authority_state"}; _require_exact_keys(parsed,keys,"handoff_receipt")
    if parsed["receipt_index"]!=46 or parsed["receipt_kind"]!="qnty_cross_agent_handoff_receipt" or parsed["schema_version"]!="0.1.0":_fail("v046 dispatcher repair receipt structure is wrong")
    ...
    if len(parsed["next_actions"])!=1:_fail("v046 dispatcher repair needs exactly one next action")
def _validate_receipt(parsed,active,root):
    if active["phase"]!=_v046.PHASE:return _validate_receipt_v041(parsed,active,root)
    _validate_v046_receipt_body(parsed)
    if parsed["task_id"]!=active["task_id"] or parsed["protocol_id"]!=active["protocol_id"]:_fail("handoff_receipt identity does not match active_task")
    _v046.validate(parsed,root); _cross_check_artifact_records(parsed,root); _validate_predecessor_chain(parsed,root,active["handoff_receipt_path"]); return parsed
_render_context_packet_v041=render_context_packet
def render_context_packet(state):
    packet=_render_context_packet_v041(state)
    if state["active_task"]["phase"]!=_v046.PHASE:return packet
    lines=packet.splitlines(); i=next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")),len(lines)); lines[i:i]=["EXTERNAL_TRUST_ROOT_DISPATCHER_V1_REPAIR=CANDIDATE_FOR_INDEPENDENT_REVIEW_ONLY"]; return "\n".join(lines)
_validate_predecessor_chain_v041=_validate_predecessor_chain
def _validate_predecessor_chain(parsed,root,receipt_relpath):
    if parsed.get("receipt_index")!=46:return _validate_predecessor_chain_v041(parsed,root,receipt_relpath)
    p=parsed["predecessor"]; target=root/p["path"]
    if p["path"]!=_v046.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest()!=p["sha256"]:_fail("v046 dispatcher repair predecessor chain is wrong")
    _validate_predecessor_chain_v041(_load_canonical_document(target.read_bytes(),"predecessor receipt"),root,p["path"])
# END EXTERNAL TRUST-ROOT DISPATCHER REPAIR CANDIDATE V046 APPEND
# BEGIN EXTERNAL TRUST-ROOT DISPATCHER REPAIR FAILURE V047 APPEND
from quantbot.continuity import external_trust_root_dispatcher_repair_failure_v047 as _v047
_validate_receipt_v046 = _validate_receipt
def _validate_v047_receipt_body(parsed):
    keys=set(_RECEIPT_KEYS)|{"phase","current_transition_files","numerical_convention_gap_inventory","numerical_conventions_selected_convention_inventory","rng_runtime_candidate_resolved_inventory","engine_implementation_binding","per_run_coordinate_and_seed_orchestration_binding","v039_checkout_path_repair","v040_review_binding","v045_failed_review_binding","construction_lineage","authority_state","v046_failed_review_binding"}; _require_exact_keys(parsed,keys,"handoff_receipt")
    if parsed["receipt_index"]!=47 or parsed["receipt_kind"]!="qnty_cross_agent_handoff_receipt" or parsed["schema_version"]!="0.1.0":_fail("v047 dispatcher failure receipt structure is wrong")
    _require_str_list(parsed["changed_file_scope"],"handoff_receipt changed_file_scope",minimum=1); _require_str_list(parsed["next_actions"],"handoff_receipt next_actions",minimum=1)
    if len(parsed["next_actions"])!=1:_fail("v047 dispatcher failure needs exactly one next action")
def _validate_receipt(parsed,active,root):
    if active["phase"]!=_v047.PHASE:return _validate_receipt_v046(parsed,active,root)
    _validate_v047_receipt_body(parsed)
    if parsed["task_id"]!=active["task_id"] or parsed["protocol_id"]!=active["protocol_id"]:_fail("handoff_receipt identity does not match active_task")
    _v047.validate(parsed,root); _cross_check_artifact_records(parsed,root); _validate_predecessor_chain(parsed,root,active["handoff_receipt_path"]); return parsed
_render_context_packet_v046=render_context_packet
def render_context_packet(state):
    packet=_render_context_packet_v046(state)
    if state["active_task"]["phase"]!=_v047.PHASE:return packet
    lines=packet.splitlines(); i=next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")),len(lines)); lines[i:i]=["EXTERNAL_TRUST_ROOT_DISPATCHER_V1_REPAIR=V046_REPAIR_REQUIRED_NOT_RELEASED"] ; return "\n".join(lines)
_validate_predecessor_chain_v046=_validate_predecessor_chain
def _validate_predecessor_chain(parsed,root,receipt_relpath):
    if parsed.get("receipt_index")!=47:return _validate_predecessor_chain_v046(parsed,root,receipt_relpath)
    p=parsed["predecessor"]; target=root/p["path"]
...
# BEGIN EXTERNAL TRUST-ROOT DISPATCHER REPAIR FAILURE V049 APPEND
from quantbot.continuity import external_trust_root_dispatcher_repair_failure_v049 as _v049
_validate_receipt_v048 = _validate_receipt
def _validate_v049_receipt_body(parsed):
    keys=set(_RECEIPT_KEYS)|{"phase","current_transition_files","numerical_convention_gap_inventory","numerical_conventions_selected_convention_inventory","rng_runtime_candidate_resolved_inventory","engine_implementation_binding","per_run_coordinate_and_seed_orchestration_binding","v039_checkout_path_repair","v040_review_binding","v045_failed_review_binding","construction_lineage","authority_state","v046_failed_review_binding","v048_failed_review_binding"}; _require_exact_keys(parsed,keys,"handoff_receipt")
    if parsed["receipt_index"]!=49 or parsed["receipt_kind"]!="qnty_cross_agent_handoff_receipt" or parsed["schema_version"]!="0.1.0":_fail("v049 dispatcher failure receipt structure is wrong")
    _require_str_list(parsed["changed_file_scope"],"handoff_receipt changed_file_scope",minimum=1); _require_str_list(parsed["next_actions"],"handoff_receipt next_actions",minimum=1)
    if len(parsed["next_actions"])!=1:_fail("v049 dispatcher failure needs exactly one next action")
def _validate_receipt(parsed,active,root):
    if active["phase"]!=_v049.PHASE:return _validate_receipt_v048(parsed,active,root)
    _validate_v049_receipt_body(parsed)
    if parsed["task_id"]!=active["task_id"] or parsed["protocol_id"]!=active["protocol_id"]:_fail("handoff_receipt identity does not match active_task")
    _v049.validate(parsed,root); _cross_check_artifact_records(parsed,root); _validate_predecessor_chain(parsed,root,active["handoff_receipt_path"]); return parsed
_render_context_packet_v048=render_context_packet
def render_context_packet(state):
    packet=_render_context_packet_v048(state)
    if state["active_task"]["phase"]!=_v049.PHASE:return packet
    lines=packet.splitlines(); i=next((i for i,line in enumerate(lines) if line.startswith("PROHIBITED=")),len(lines)); lines[i:i]=["EXTERNAL_TRUST_ROOT_DISPATCHER_V1_REPAIR=V048_REPAIR_REQUIRED_NOT_RELEASED"] ; return "\n".join(lines)
_validate_predecessor_chain_v048=_validate_predecessor_chain
def _validate_predecessor_chain(parsed,root,receipt_relpath):
    if parsed.get("receipt_index")!=49:return _validate_predecessor_chain_v048(parsed,root,receipt_relpath)
    p=parsed["predecessor"]; target=root/p["path"]
    if p["path"]!=_v049.previous.HANDOFF_RELPATH or not target.is_file() or hashlib.sha256(target.read_bytes()).hexdigest()!=p["sha256"]:_fail("v049 dispatcher failure predecessor chain is wrong")
    _validate_predecessor_chain_v048(_load_canonical_document(target.read_bytes(),"predecessor receipt"),root,p["path"])
# END EXTERNAL TRUST-ROOT DISPATCHER REPAIR FAILURE V049 APPEND
