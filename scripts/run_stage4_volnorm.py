#!/usr/bin/env python3
"""Stage 4 — Multi-Asset TSMOM Walkforward with Vol-Normalized Portfolio.
...
3. Uses volatility-normalized position weights with portfolio heat cap
4. Computes per-split: total return, Sharpe, max drawdown, win rate, funding cost, excess return
5. Aggregates across splits (simple average, equal weight per split)
6. Applies portfolio-level kill criteria (K1–K4; K5/K6 diagnostic)
...
class SplitEquity:
    ...
    strat_net: list[float]   # net-of-carry per-bar log returns (strategy, weighted)
    bench: list[float]       # per-bar log returns (benchmark, always-long, equal-weight)
    ...
    # Heat metrics
    avg_portfolio_heat: float = 0.0
    heat_cap_applications: int = 0
    bar_count: int = 0
    ...
    def compute(self) -> None:
        ...
        self.bar_count = len(self.strat_net)
...
class AggregatedMetrics:
    ...
    avg_portfolio_heat_avg: float = 0.0
    heat_cap_applications_total: int = 0
...
def evaluate_split_equity(
    ...
    Uses volatility-normalized position weights with portfolio heat cap.
    Restructures the per-bar loop to:
    1. First process all symbols, collecting returns per regime
    2. Then compute vol-normalized weights for active symbols
    3. Then append weighted returns to the regime lists
    """
    ...
    # Initialize volatility trackers for each regime
    # These track volatility across the FULL test period (train history flows into them)
    vol_trackers_low: dict[str, VolatilityTracker] = {
        symbol: VolatilityTracker(lookback=VOL_LOOKBACK_BARS)
        for symbol in universe if symbol in test_bars_by_symbol
    }
    vol_trackers_high: dict[str, VolatilityTracker] = {
        symbol: VolatilityTracker(lookback=VOL_LOOKBACK_BARS)
        for symbol in universe if symbol in test_bars_by_symbol
    }

    # Pre-load training bars to warm up volatility trackers before test period
    for symbol in universe:
        if symbol not in train_bars_by_symbol:
            continue
        train_bars = train_bars_by_symbol[symbol]
        prev_close_train = None
        for bar in train_bars:
            if prev_close_train is not None:
                ret = _log_return(prev_close_train, bar.close)
                # Update both regime trackers with training data
                if symbol in vol_trackers_low:
                    vol_trackers_low[symbol].update(ret)
                if symbol in vol_trackers_high:
                    vol_trackers_high[symbol].update(ret)
            prev_close_train = bar.close

    # Per-bar return storage: we collect all symbol returns first, then weight
    # Structure: per-bar dict of {symbol: (ret, ret_net)} for each regime
    low_vol_bar_data: list[dict[str, tuple[float, float]]] = []
    high_vol_bar_data: list[dict[str, tuple[float, float]]] = []
    ...
    for i in range(min_test_len):
        # First pass: compute returns for all symbols this bar, update vol trackers
        low_symbols_this_bar: dict[str, tuple[float, float]] = {}
        high_symbols_this_bar: dict[str, tuple[float, float]] = {}
        ...
        for symbol in universe:
            ...
            # Update volatility trackers with this bar's return (for next bar's weight)
            if regime == "low_vol" and symbol in vol_trackers_low:
                vol_trackers_low[symbol].update(ret)
            elif regime == "high_vol" and symbol in vol_trackers_high:
                vol_trackers_high[symbol].update(ret)
            ...
            if current_regime == "low_vol":
                ...
                # Collect return for later weighting
                if signal is not None and signal.direction == "long":
                    low_symbols_this_bar[symbol] = (ret, ret_net)
            elif current_regime == "high_vol":
                bench_high.append(ret)
                # Collect return for later weighting
                if signal is not None and signal.direction == "long":
                    high_symbols_this_bar[symbol] = (ret, ret_net)

            prev_close[symbol] = bar.close

        # Store per-regime bar data
        low_vol_bar_data.append(low_symbols_this_bar)
        high_vol_bar_data.append(high_symbols_this_bar)

    # Second pass: compute weighted returns per regime
    strat_low_net: list[float] = []
    strat_high_net: list[float] = []
    heat_cap_count_low = 0
    heat_cap_count_high = 0
    total_heat_low = 0.0
    total_heat_high = 0.0
    bars_with_active_low = 0
    bars_with_active_high = 0

    # Low-vol regime
    for bar_data in low_vol_bar_data:
        if not bar_data:
            # No active positions this bar
            strat_low_net.append(0.0)
            continue

        active_symbols = list(bar_data.keys())
        weights = compute_vol_normed_weights(active_symbols, vol_trackers_low, HEAT_CAP)

        # Compute portfolio heat for this bar
        bar_heat = sum(
            weights[s] * vol_trackers_low[s].volatility
            for s in active_symbols if s in vol_trackers_low
        )
        total_heat_low += bar_heat
        bars_with_active_low += 1

        # Check if heat cap was applied
        if bar_heat > HEAT_CAP and bar_heat > 0:
            heat_cap_count_low += 1

        # Apply weighted returns
        weighted_ret = sum(bar_data[s][1] * weights[s] for s in active_symbols)
        strat_low_net.append(weighted_ret)

    # High-vol regime
    for bar_data in high_vol_bar_data:
        if not bar_data:
            # No active positions this bar
            strat_high_net.append(0.0)
            continue

        active_symbols = list(bar_data.keys())
        weights = compute_vol_normed_weights(active_symbols, vol_trackers_high, HEAT_CAP)

        # Compute portfolio heat for this bar
        bar_heat = sum(
            weights[s] * vol_trackers_high[s].volatility
            for s in active_symbols if s in vol_trackers_high
        )
        total_heat_high += bar_heat
        bars_with_active_high += 1

        # Check if heat cap was applied
        if bar_heat > HEAT_CAP and bar_heat > 0:
            heat_cap_count_high += 1

        # Apply weighted returns
        weighted_ret = sum(bar_data[s][1] * weights[s] for s in active_symbols)
        strat_high_net.append(weighted_ret)

    all_strat_net = strat_low_net + strat_high_net
    all_bench = bench_low + bench_high

    # Compute heat metrics
    avg_heat_low = total_heat_low / max(1, bars_with_active_low)
    avg_heat_high = total_heat_high / max(1, bars_with_active_high)
    total_bars_with_active = bars_with_active_low + bars_with_active_high
    avg_heat_all = (total_heat_low + total_heat_high) / max(1, total_bars_with_active)
    total_heat_cap_applications = heat_cap_count_low + heat_cap_count_high
    ...
    e_all = SplitEquity(
        ...
        avg_portfolio_heat=avg_heat_all,
        heat_cap_applications=total_heat_cap_applications,
    )
    e_low = SplitEquity(
        ...
        avg_portfolio_heat=avg_heat_low,
        heat_cap_applications=heat_cap_count_low,
    )
    e_high = SplitEquity(
        ...
        avg_portfolio_heat=avg_heat_high,
        heat_cap_applications=heat_cap_count_high,
    )
...
def aggregate(equities: list[SplitEquity]) -> AggregatedMetrics:
    if not equities:
        return AggregatedMetrics(
            ...
            avg_portfolio_heat_avg=0.0, heat_cap_applications_total=0,
        )
    n = len(equities)
    return AggregatedMetrics(
        split_count=n,
        ...
        avg_portfolio_heat_avg=sum(e.avg_portfolio_heat for e in equities) / n,
        heat_cap_applications_total=sum(e.heat_cap_applications for e in equities),
    )
...
def write_csv(equities: list[SplitEquity], output_path: Path) -> None:
    fieldnames = [
        ...
        "avg_portfolio_heat", "heat_cap_applications",
    ]
    with open(output_path, "w", newline="") as f:
        ...
        for e in sorted(equities, key=lambda x: (x.split_index, x.regime, x.return_period, x.threshold)):
            writer.writerow({
                ...
                "avg_portfolio_heat": round(e.avg_portfolio_heat, 6),
                "heat_cap_applications": e.heat_cap_applications,
            })


def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 4 Vol-Normalized Walkforward")
    parser.add_argument("--output-dir", default="output/stage4_volnorm")
    ...
    print(f"\n{'='*60}")
    print("STAGE 4 — VOL-NORMALIZED WALKFORWARD EXTENSION")
    print(f"{'='*60}")
    print(f"Position sizing: volatility-normalized (inverse-vol)")
    print(f"Portfolio heat cap: {HEAT_CAP}")
    print(f"Vol lookback: {VOL_LOOKBACK_BARS} bars")
    ...
    # Write package identity
    package_identity_path = output_dir / "package_identity.json"
    with open(package_identity_path, "w") as f:
        json.dump(PACKAGE_IDENTITY, f, indent=2)
    print(f"Package identity JSON: {package_identity_path}")

    # Summary table
    print(f"\n{'='*60}")
    print(f"PER-SPLIT SUMMARY (vol-normalized, all grid points)")
    print(f"{'='*60}")
    print(f"{'Split':>5} {'RP':>3} {'Th':>4} {'Regime':>8} | {'NetRet':>8} {'Sharpe':>7} {'MaxDD':>7} {'WinRate':>8} | {'BenchRet':>8} {'Excess':>8} | {'AvgHeat':>7} {'HeatCap':>7}")
    print("-" * 110)
    for e in sorted(all_eq, key=lambda x: (x.split_index, x.return_period, x.threshold, x.regime)):
        print(f"  {e.split_index:>3} {e.return_period:>3} {e.threshold:>4.2f} {e.regime:>8} | "
              ...
              f"{e.avg_portfolio_heat:>7.4f} {e.heat_cap_applications:>7}")
    ...
    print(f"  Avg portfolio heat:  {agg_all.avg_portfolio_heat_avg:.4f}")
    print(f"  Heat cap apps (tot): {agg_all.heat_cap_applications_total}")
    ...
    print(f"  Avg portfolio heat:  {agg_highvol.avg_portfolio_heat_avg:.4f}")
    print(f"  Heat cap apps (tot): {agg_highvol.heat_cap_applications_total}")
