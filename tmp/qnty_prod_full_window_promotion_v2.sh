...
echo "--- Step 2: Setup scratch checkout ---"
SCRATCH=$(mktemp -d)
cd "$SCRATCH"
git clone /srv/qnty/repo . 2>&1 | tail -1
if [ ! -f "quantbot/paper/sqlite_verify.py" ]; then
    echo "FAIL: Scratch checkout incomplete"; exit 1
fi
echo "PASS: Scratch checkout at ${SCRATCH}"
...
if [ -n "$UNWANTED" ]; then
    echo "BLOCKED: Unwanted processes running:"
    echo "$UNWANTED"; exit 1
fi
echo "PASS: No unwanted processes"

echo "--- Step 4: Generate candidate ---"
...
if [ ! -f "$CANDIDATE" ]; then
    echo "FAIL: Candidate file not created"
    echo "FINAL_VERDICT=QNTY_PROD_FULL_WINDOW_REPORT_PROMOTION_EXECUTION_V2_BLOCKED"
...
if [ "$CANDIDATE_SIZE" -lt 100 ]; then
    echo "FAIL: Candidate too small (${CANDIDATE_SIZE} bytes)"
    cat "$CANDIDATE"; cat "$STDERR_FILE"
    echo "FINAL_VERDICT=QNTY_PROD_FULL_WINDOW_REPORT_PROMOTION_EXECUTION_V2_BLOCKED"
    echo "REASON=candidate_too_small"
    rm -rf "$SCRATCH"; exit 1
fi
...
checks = [
    ...
    ('funding_clean_carry_decision', cand.get('funding_clean_carry_decision'), 'CLEAN_NET_OF_CARRY'),
]
for name, val, expected in checks:
    ok = val == expected
    print(('PASS' if ok else 'FAIL') + ': ' + name + ' = ' + str(val))

pub_keys = ['authoritative', 'trusted', 'content_digests', 'content_sha256', 'snapshot_identity', 'verifier']
for k in pub_keys:
    ok = k in cand
...
if [ "$VALIDATION_EXIT" -ne 0 ]; then
    ...
    echo "FINAL_VERDICT=QNTY_PROD_FULL_WINDOW_REPORT_PROMOTION_EXECUTION_V2_BLOCKED"
    echo "REASON=candidate_validation_failed"
    rm -rf "$SCRATCH"; exit 1
fi
...
POSTFLIGHT_CSV_HASHES=$(find "$DATA_DIR" -name '*.csv' -type f | sort | xargs sha256sum 2>/dev/null)
...
if [ "$PREFLIGHT_CSV_AGG" != "$POSTFLIGHT_CSV_AGG" ]; then echo "FAIL: CSV hashes changed"; exit 1; fi
echo "PASS: CSV hashes unchanged"

POSTFLIGHT_SNAPSHOT_HASHES=$(find "$SNAPSHOT_DIR" -type f | sort | xargs sha256sum 2>/dev/null)
...
if [ "$PREFLIGHT_SNAPSHOT_AGG" != "$POSTFLIGHT_SNAPSHOT_AGG" ]; then echo "FAIL: Snapshot hashes changed"; exit 1; fi
echo "PASS: Snapshot hashes unchanged"

POSTFLIGHT_BUNDLE_HASHES=$(find "$BUNDLE_DIR" -type f | sort | xargs sha256sum 2>/dev/null)
...
if [ "$PREFLIGHT_BUNDLE_AGG" != "$POSTFLIGHT_BUNDLE_AGG" ]; then echo "FAIL: Bundle hashes changed"; exit 1; fi
